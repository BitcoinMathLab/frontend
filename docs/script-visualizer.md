...
The Script Visualizer at `/visualizer` teaches one curated P2PKH spend and accepts a compatible Explorer selection at
...
The source bar above the workspace also accepts a transaction ID and zero-based input directly, so an arbitrary legacy
P2PKH spend can be explored without constructing a URL.
...
6. Select `scriptSig` and `scriptPubKey` themselves. Expect source dialogs identifying the spending transaction input
   and previous transaction output. At `OP_CHECKSIG`, open the Signature workspace and inspect the real preimage,
   sighash type, digest, signature, public key, and verification result. The Execution/Signature tabs must respond to
...
8. From Explorer, inspect a transaction with a P2PKH input and activate `Visualize this spend`. Expect the visualizer
   source transaction and input to match the Explorer result. Non-P2PKH inputs must not show this action.
9. Advance through the last opcode. The final playback phase must read `STACK VALIDATION` and
   explain that the spend is valid only when the final stack value is true. The verdict must not
   appear before this phase.
10. Exercise the failed-trace response. Expect `Invalid spend` and a safe failure explanation only
    at the final validation phase; do not expect engine internals, credentials, or a traceback.
11. Repeat the walkthrough at a 390-pixel viewport and with reduced motion enabled. Expect no
